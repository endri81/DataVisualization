...
% % ═══════════════════════════════════════════════════════
%  MODULE 3.10 — Workshop 3 Summary and Comprehensive Reference
% ═══════════════════════════════════════════════════════
\section{Module 3.10 --- Workshop Summary and Reference}

% ── 272 ──
\begin{frame}{Module 3.10 --- Workshop 3 Recap}\smark{272/300}
\begin{enumerate}
    \item Module 3.1 --- Bars, proportions, mosaic, waffle, lollipop
    \item Module 3.2 --- $\chi^2$, Cram\'er's $V$, residual shading
    \item Module 3.3 --- Alluvial, Sankey, bump, slope, UpSet
    \item Module 3.4 --- Likert, diverging stacked, NPS
    \item Module 3.5 --- Correspondence analysis and MCA biplots
    \item Module 3.6 --- Ordinal data, cumulative CDFs, proportional odds
    \item Module 3.7 --- Fourfold, log-linear, doubledecker, agreement
    \item Module 3.8 --- Case studies and multi-panel dashboards
    \item Module 3.9 --- Text as categorical data
    \item Module 3.10 --- This summary
\end{enumerate}
\end{frame}

% ── 273 ──
\begin{frame}{The Categorical Visualization Hierarchy}
\smark{273/300}
\centering\small
\begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Level} & \textbf{What You Show} & \textbf{Key Charts} \\
    \midrule
    1. Counts           & How many per category      & Bar (sorted), lollipop \\
    2. Proportions      & Part-of-whole              & Stacked bar, waffle, spine \\
    3. Association      & Do variables relate?       & Mosaic + residual shading \\
    4. Flow             & How entities partition      & Alluvial, Sankey, UpSet \\
    5. Ordinal shape    & Distribution + order        & Diverging stacked, CDF \\
    6. Latent structure & Dimensions of association   & CA biplot, MCA, LDA topics \\
    7. Multi-panel      & Complete analytical story   & Dashboard composites \\
    \bottomrule
\end{tabular}

\vspace{0.3cm}
\begin{exampleblock}{Data Insight}
    \scriptsize Workshop 3 covered all seven levels. Level 1--2 are descriptive; 3--4 are relational; 5 respects ordinal structure; 6 discovers hidden dimensions; 7 assembles the story for publication.
\end{exampleblock}
\end{frame}

% ── 274 ──
\begin{frame}{Master Chart Decision Guide --- Categorical Data}
\smark{274/300}
\centering\scriptsize
\begin{tabular}{@{}llll@{}}
    \toprule
    \textbf{Question} & \textbf{Chart} & \textbf{Module} & \textbf{Key Package} \\
    \midrule
    How many per category?           & Bar (sorted)                 & 3.1 & \texttt{geom\_bar} \\
    What share?                      & Waffle, 100\% stacked        & 3.1 & \texttt{waffle} \\
    Is there association?            & Mosaic + residuals            & 3.2 & \texttt{vcd::mosaic} \\
    How strong?                      & Cram\'er's $V$ or OR          & 3.2, 3.7 & \texttt{effectsize} \\
    How do subjects flow?            & Alluvial / Sankey             & 3.3 & \texttt{ggalluvial}, \texttt{plotly} \\
    Set intersections?               & UpSet plot                    & 3.3 & \texttt{UpSetR} \\
    Rank changes?                    & Bump chart                    & 3.3 & \texttt{ggbump} \\
    Likert responses?                & Diverging stacked + net       & 3.4 & \texttt{likert} \\
    Latent dimensions?               & CA / MCA biplot               & 3.5 & \texttt{FactoMineR} \\
    Ordinal comparison?              & Cumulative CDF                & 3.6 & Manual \\
    $2 \times 2$ effect?             & Fourfold + OR forest          & 3.7 & \texttt{vcd}, \texttt{meta} \\
    Binary outcome by predictor?     & Doubledecker / CD plot        & 3.7 & \texttt{vcd} \\
    Text themes?                     & LDA bars + LDAvis             & 3.9 & \texttt{topicmodels} \\
    Sentiment?                       & Diverging sentiment bar       & 3.9 & \texttt{tidytext}, VADER \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 275 ──
\begin{frame}{Workshop 1 vs.\ 2 vs.\ 3}
\smark{275/300}
\centering\small
\begin{tabular}{@{}llll@{}}
    \toprule
    \textbf{Dimension} & \textbf{W1: Foundations} & \textbf{W2: Statistical} & \textbf{W3: Categorical} \\
    \midrule
    Data type    & Continuous           & Continuous + model      & Categorical + ordinal \\
    Key geoms    & point, col, line     & violin, slab, errorbar  & bar, tile, mosaic \\
    Key concepts & Encoding, Gestalt    & CI, effect size, resid  & $\chi^2$, OR, profiles \\
    Dimension red.& ---                 & ---                     & CA, MCA, LDA \\
    Tests visual.& ---                  & $t$, ANOVA, ROC         & $\chi^2$, Fisher, KW \\
    Flow charts  & ---                  & ---                     & Alluvial, Sankey, UpSet \\
    Text         & ---                  & ---                     & Freq bars, topics, sentiment \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 276 ──
\begin{frame}{Master Package Reference --- Categorical (\Rlang)}
\smark{276/300}
\centering\scriptsize
\begin{tabular}{@{}lll@{}}
    \toprule
    ...
    \midrule
    Mosaic, assoc, sieve     & \texttt{vcd}            & \texttt{mosaic()}, \texttt{assoc()}, \texttt{sieve()} \\
    Fourfold, doubledecker   & \texttt{vcd}            & \texttt{fourfoldplot()}, \texttt{doubledecker()} \\
    Alluvial                 & \texttt{ggalluvial}     & \texttt{geom\_alluvium()} \\
    Sankey                   & \texttt{networkD3}      & \texttt{sankeyNetwork()} \\
    Bump chart               & \texttt{ggbump}         & \texttt{geom\_bump()} \\
    UpSet                    & \texttt{UpSetR}         & \texttt{upset()} \\
    Likert                   & \texttt{likert}         & \texttt{plot(likert())} \\
    Waffle                   & \texttt{waffle}         & \texttt{waffle()} \\
    CA / MCA                 & \texttt{FactoMineR}     & \texttt{CA()}, \texttt{MCA()} \\
    CA visualization         & \texttt{factoextra}     & \texttt{fviz\_ca\_biplot()} \\
    Treemap                  & \texttt{treemapify}     & \texttt{geom\_treemap()} \\
    Text analysis            & \texttt{tidytext}       & \texttt{unnest\_tokens()}, \texttt{bind\_tf\_idf()} \\
    Topic models             & \texttt{topicmodels}    & \texttt{LDA()} \\
    Text network             & \texttt{ggraph}         & \texttt{geom\_edge\_link()} \\
    Meta-analysis forest     & \texttt{meta}           & \texttt{forest()} \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 277 ──
\begin{frame}{Master Package Reference --- Categorical (\Python)}
\smark{277/300}
\centering\scriptsize
\begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Task} & \textbf{Package} & \textbf{Key Function} \\
    \midrule
    Mosaic                   & \texttt{statsmodels}    & \texttt{mosaicplot.mosaic()} \\
    Sankey / alluvial        & \texttt{plotly}         & \texttt{go.Sankey()}, \texttt{px.parallel\_categories()} \\
    Parallel sets            & \texttt{plotly.express} & \texttt{parallel\_categories()} \\
    UpSet                    & \texttt{upsetplot}      & \texttt{UpSet()} \\
    Likert                   & \texttt{plot-likert}    & \texttt{plot\_likert()} \\
    Waffle                   & \texttt{pywaffle}       & \texttt{Waffle()} \\
    CA / MCA                 & \texttt{prince}         & \texttt{CA()}, \texttt{MCA()} \\
    Treemap                  & \texttt{squarify}       & \texttt{squarify.plot()} \\
    Word cloud               & \texttt{wordcloud}      & \texttt{WordCloud()} \\
    TF-IDF                   & \texttt{sklearn}        & \texttt{TfidfVectorizer} \\
    Topic models             & \texttt{sklearn}        & \texttt{LatentDirichletAllocation} \\
    LDAvis                   & \texttt{pyLDAvis}       & \texttt{prepare()} \\
    Sentiment                & \texttt{vaderSentiment} & \texttt{SentimentIntensityAnalyzer} \\
    Network                  & \texttt{networkx}       & \texttt{nx.draw()} \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 278 ──
\begin{frame}{Categorical Anti-Pattern Summary}
\smark{278/300}
\centering\small
\begin{tabular}{@{}ll@{}}
    \toprule
    \textbf{Anti-Pattern} & \textbf{Better Alternative} \\
    \midrule
    Pie chart ($> 3$ slices)                  & Waffle or stacked bar \\
    3D anything                               & Flat 2D always \\
    Alphabetical nominal order                & Sort by value \\
    Bar chart for association                  & Mosaic + residual shading \\
    $\chi^2$ without effect size              & Add Cram\'er's $V$ or OR \\
    Mean $\pm$ SD for Likert/ordinal          & Diverging stacked + median \\
    Venn for 4+ sets                          & UpSet plot \\
    Red-green only                            & Okabe-Ito or direct labels \\
    Word cloud as sole analysis               & Frequency bar for precision \\
    Spaghetti alluvial                        & Aggregate small flows to ``Other'' \\
    Unweighted $\kappa$ for ordinal           & Weighted $\kappa$ (quadratic) \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 279 ──
\begin{frame}{Statistical Test $\to$ Visualization Mapping}
\smark{279/300}
\centering\small
\begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Test} & \textbf{Visualization} & \textbf{Effect Size} \\
    \midrule
    $\chi^2$ independence     & Mosaic + residual shading   & Cram\'er's $V$ \\
    Fisher's exact            & Mosaic + residuals           & OR + CI \\
    McNemar's (paired)        & Before/after table heatmap   & OR \\
    CMH (stratified)          & Conditional mosaics          & Common OR \\
    Kruskal--Wallis           & Ridgeline or cumulative CDF  & Epsilon-squared \\
    Mann--Whitney $U$         & Estimation plot              & Cliff's $\delta$ / CLES \\
    Proportional odds         & Cumulative log-odds          & OR per threshold \\
    Log-linear                & Mosaic of residuals          & $G^2$ / AIC \\
    Cohen's $\kappa$          & Agreement heatmap            & $\kappa$ + CI \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 280 ──
\begin{frame}{The 6-Step Workflow Revisited}
\smark{280/300}
\centering\small
\begin{tabular}{@{}lll@{}}
    \toprule
    ...
    \midrule
    1. Tabulate       & \texttt{table()} / \texttt{pd.crosstab()}  & Contingency table \\
    2. Test           & $\chi^2$, Fisher, KW, CMH                 & $p$-value \\
    3. Effect size    & Cram\'er's $V$, OR, RR, CLES              & Magnitude + CI \\
    4. Visualize      & Mosaic, alluvial, diverging bar            & Figure \\
    5. Decompose      & Residuals, contributions, CA               & Cell-level insight \\
    6. Report         & Figure + caption + effect + $N$            & Publication-ready \\
    \bottomrule
\end{tabular}

\vspace{0.3cm}
\begin{alertblock}{Pro-Tip}
    \scriptsize The workflow is iterative: step 5 (decompose) often sends you back to step 4 (visualize a sub-table or conditional mosaic). Steps 1--3 are computation; 4--6 are communication. Most analysts skip 4--6.
\end{alertblock}
\end{frame}

% ── 281 ──
\begin{frame}{Image Manifest: Workshop 3}
\smark{281/300}
\centering\scriptsize
\begin{tabular}{@{}lll@{}}
    \toprule
    ...
    \midrule
    3.1 Bars \& proportions & \texttt{m3\_1\_stacked\_bar}, \texttt{m3\_1\_mosaic}, \ldots & 7 \\
    3.2 Association         & \texttt{m3\_2\_pearson\_resid}, \texttt{m3\_2\_mosaic\_shaded}, \ldots & 7 \\
    3.3 Flow diagrams       & \texttt{m3\_3\_alluvial}, \texttt{m3\_3\_sankey}, \ldots & 6 \\
    3.4 Likert \& survey    & \texttt{m3\_4\_diverging\_stacked}, \texttt{m3\_4\_nps}, \ldots & 7 \\
    3.5 Correspondence analysis & \texttt{m3\_5\_ca\_biplot}, \texttt{m3\_5\_mca\_biplot}, \ldots & 6 \\
    3.6 Ordinal data        & \texttt{m3\_6\_cumulative}, \texttt{m3\_6\_cum\_odds}, \ldots & 7 \\
    3.7 Advanced tables     & \texttt{m3\_7\_fourfold}, \texttt{m3\_7\_or\_forest}, \ldots & 6 \\
    3.8 Case studies        & \texttt{m3\_8\_clinical\_dashboard}, \texttt{m3\_8\_workflow}, \ldots & 4 \\
    3.9 Text as categorical & \texttt{m3\_9\_word\_freq}, \texttt{m3\_9\_topics}, \ldots & 7 \\
    \midrule
...
% ── 282 ──
\begin{frame}{Key References --- Categorical Visualization}
\smark{282/300}
\centering\scriptsize
\begin{tabular}{@{}ll@{}}
    \toprule
    ...
    \midrule
    Mosaic plots, \texttt{vcd}    & Friendly (2000), \textit{Visualizing Categorical Data} \\
    Association plots              & Friendly (1992), \textit{American Statistician} \\
    Correspondence analysis        & Greenacre (2017), \textit{Correspondence Analysis in Practice} \\
    Raincloud plots                & Allen et al.\ (2019), \textit{Wellcome Open Research} \\
    LDAvis                         & Sievert \& Shirley (2014), \textit{ACL Workshop} \\
    Narrative arcs                 & Reagan et al.\ (2016), \textit{EPJ Data Science} \\
    UpSet plots                    & Lex et al.\ (2014), \textit{IEEE TVCG} \\
    Zipf's law                     & Zipf (1935), \textit{The Psycho-Biology of Language} \\
    Likert visualization           & Robbins \& Heiberger (2011), \textit{Survey Analysis} \\
    \bottomrule
\end{tabular}
\end{frame}
...
\begin{column}{0.48\textwidth}
    \textbf{Books:}
    \begin{itemize}
        \item Friendly \& Meyer (2016), \textit{Discrete Data Analysis with R}
        \item Agresti (2013), \textit{Categorical Data Analysis}
        \item Greenacre (2017), \textit{Correspondence Analysis in Practice}
        \item Silge \& Robinson (2017), \textit{Text Mining with R}
    \end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
    \textbf{Online:}
    \begin{itemize}
        \item \texttt{cran.r-project.org/package=vcd} --- vcd vignettes
        \item \texttt{juliasilge.github.io/tidytext} --- tidytext book
        \item \texttt{prince.readthedocs.io} --- prince CA/MCA docs
        \item \texttt{pyLDAvis.readthedocs.io} --- interactive topics
    \end{itemize}
\end{column}
\end{columns}
\end{frame}
...
% ── 284 ──
\begin{frame}{Workshop 3 Skills Acquired}
\smark{284/300}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
    \textbf{You can now:}
    \begin{itemize}
        \item Build stacked, dodged, 100\% bars with proper sorting and annotation
        \item Construct mosaic plots with residual shading from $\chi^2$ tests
        \item Create alluvial, Sankey, bump, and slope charts for categorical flow
        \item Visualize Likert data with diverging stacked bars, net scores, and NPS
    \end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
    \begin{itemize}
        \item Perform and visualize CA and MCA with biplots, scree, and cos$^2$
        \item Handle ordinal data with CDFs, median+IQR, and proportional odds checks
        \item Use fourfold, doubledecker, CD plots, and agreement displays
        \item Assemble multi-panel publication dashboards
        \item Visualize text as categorical: frequency bars, TF-IDF, LDA topics, sentiment
    \end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\smark{285/300}
\begin{enumerate}
    \item \textbf{Sort nominal by value}; preserve ordinal order
    \item \textbf{Area or length for proportion}; never angle (pie)
    \item \textbf{Residual shading} integrates the $\chi^2$ test into the display
    \item \textbf{Diverging palette} = semantic: red for disagree/negative, blue for agree/positive
    \item \textbf{Effect size always}: Cram\'er's $V$, OR, CLES --- not just $p$
    \item \textbf{Show the full distribution}, not just the summary
    \item \textbf{Aggregate small categories} into ``Other'' before plotting
    \item \textbf{Colorblind-safe palettes} with secondary encoding (shape, texture, labels)
\end{enumerate}
\end{frame}

% ── 286 ──
\begin{frame}{Overleaf Assembly: Workshop 3}
\smark{286/300}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
    \textbf{Files to upload:}
    \begin{enumerate}
        \item \texttt{Workshop3\_CatViz\_base.tex}
        \item \texttt{W3\_Module\_3\_1} through \texttt{W3\_Module\_3\_10} (\texttt{.tex})
        \item All \texttt{m3\_*.png} files (project root)
    \end{enumerate}
\end{column}
\begin{column}{0.47\textwidth}
    \textbf{Assembly:}
    \begin{enumerate}
        \item Open \texttt{Workshop3\_CatViz\_base.tex}
        \item Paste Module 3.1 before \texttt{\textbackslash end\{document\}}
        \item Paste Modules 3.2--3.10 in order
        \item Module 3.10 contains \texttt{\textbackslash end\{document\}}
    \end{enumerate}
\end{column}
\end{columns}
...
\vspace{0.3cm}
\begin{alertblock}{Pro-Tip}
    \scriptsize The preamble is identical to Workshops 1 and 2 (same colors, listings, commands) with only the title changed. All 57+ PNGs go in the project root.
\end{alertblock}
\end{frame}
...
\begin{column}{0.48\textwidth}
    ...
    \begin{itemize}
        ...
        \item \texttt{set.seed()} for any simulation
        \item Explicit \texttt{ggsave()} / \texttt{savefig()} with dimensions + DPI
        \item Version-pinned packages
    \end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
    \textbf{Categorical-specific:}
    \begin{itemize}
        \item Store the contingency table as CSV
        \item Document category definitions (how levels were coded)
        \item Record factor level ordering (\texttt{levels=})
        \item Note any merged/collapsed categories
    \end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\smark{288/300}
\begin{enumerate}
    \item Build a residual-shaded mosaic of the \texttt{HairEyeColor} dataset. Report $\chi^2$, $p$, and Cram\'er's $V$ in the caption.
    \item Create a diverging Likert stacked bar for 5 survey questions with net scores and mean dot panel.
    \item Construct an alluvial diagram of Titanic: class $\to$ sex $\to$ survived.
    \item Fit a CA on \texttt{HairEyeColor} and produce a biplot with cos$^2$ coloring.
    \item Build a cumulative CDF comparing 3 products on a 5-point rating scale. Test with KS.
    \item Compute TF-IDF for two corpora and produce a faceted top-10 bar chart.
\end{enumerate}
\end{frame}

% ── 289 ──
\begin{frame}{What Comes Next: Workshop 4 Preview}
\smark{289/300}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
    \textbf{Workshop 4 --- Time Series and Temporal Data:}
    \begin{itemize}
        \item Line charts, step plots, area charts
        \item Seasonal decomposition visualization
        \item Autocorrelation and partial ACF
        \item Forecasting with prediction bands
        \item Calendar heatmaps
        \item Horizon charts and sparklines
    \end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
    \textbf{Workshops 5--9:}
...
% ── 290 ──
\begin{frame}{The Grammar + Statistics + Categorical Integration}
\smark{290/300}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\begin{itemize}
    \item \textbf{W1}: the grammar (data + aes + geom + stat + scale)
    \item \textbf{W2}: the statistics (what the stat layer computes)
    \item \textbf{W3}: the categories (nominal, ordinal, text as geoms and stats)
    \item Together: any visualization is a grammar pipeline applied to any data type
\end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
\begin{exampleblock}{Data Insight}
    \scriptsize A mosaic is \texttt{geom\_rect} with area $\propto$ joint frequency. A Likert diverging bar is \texttt{geom\_col} with computed centering offset. An alluvial is \texttt{geom\_alluvium} with the stat computing flow bands. The grammar unifies everything.
\end{exampleblock}
\end{column}
\end{columns}
\end{frame}
...
% ── 291 ──
\begin{frame}{Categorical Data in the Larger Curriculum}
\smark{291/300}
\centering\small
\begin{tabular}{@{}llll@{}}
    \toprule
    \textbf{Concept} & \textbf{W1} & \textbf{W2} & \textbf{W3} \\
    \midrule
    Color encoding     & Qualitative palette    & Diverging for residuals & Semantic (red/blue for Likert) \\
    Bar charts         & Basic bars             & Coefficient plot        & Stacked, dodged, diverging \\
    Heatmaps           & ---                    & Correlation             & Contingency, DTM, agreement \\
    Dimension reduction& ---                    & ---                     & CA, MCA, LDA \\
    Effect size        & ---                    & Cohen's $d$, CI         & Cram\'er's $V$, OR, CLES \\
    Diagnostics        & ---                    & Residual plots          & $\chi^2$ residuals, cos$^2$ \\
    Text               & ---                    & ---                     & Frequency, TF-IDF, topics \\
    \bottomrule
\end{tabular}
\end{frame}

\begin{frame}[fragile]{Quick-Start Template: Categorical Figure}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
...
test <- chisq.test(ct)
# 3. Effect size
V <- sqrt(test$statistic /
  (sum(ct)*(min(dim(ct))-1)))
# 4. Visualize
mosaic(ct, shade=TRUE, legend=TRUE,
  main=sprintf(
    "chi2(%d)=%.1f, p<.001, V=%.2f",
    test$parameter,
    test$statistic, V))
\end{lstlisting}
\end{column}
...
# 4. Visualize (manual mosaic)
print(f"chi2({dof})={chi2:.1f}, "
      f"p={p:.4f}, V={V:.2f}")
\end{lstlisting}
\end{column}
\end{columns}
...
% ── 293 ──
\begin{frame}{The ``One Display'' Rule for Categorical Data}
\smark{293/300}
\begin{columns}[T]
\begin{column}{0.50\textwidth}
\begin{itemize}
    \item If you could show only \textbf{one} display for a $2 \times 2$ table: \textbf{fourfold} with OR + CI
    \item For a general $r \times c$ table: \textbf{mosaic with residual shading}
    \item For Likert data: \textbf{diverging stacked bar}
    \item For flow data: \textbf{alluvial}
    \item For text data: \textbf{TF-IDF bar chart} (not word cloud)
    \item For ordinal comparison: \textbf{cumulative CDF}
\end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
\begin{exampleblock}{Data Insight}
    \scriptsize The ``one display'' answer is always the one that encodes the most information in the least space. The mosaic wins for general tables because it shows marginals, conditionals, joint frequencies, and residuals simultaneously.
\end{exampleblock}
\end{column}
\end{columns}
\end{frame}
...
% ── 294 ──
\begin{frame}{Accessibility Summary}
\smark{294/300}
\centering\small
\begin{tabular}{@{}lll@{}}
    \toprule
    \textbf{Barrier} & \textbf{Solution} & \textbf{Tool} \\
    \midrule
    Color blindness    & Okabe-Ito palette + direct labels   & \texttt{colorblindr}, \texttt{colorspacious} \\
    Screen readers     & Alt text describing patterns         & Manual \\
    Low vision         & Large fonts ($\geq 12$pt), high contrast & \texttt{theme(base\_size=14)} \\
    Texture need       & Pattern fills (stripes, dots)        & \texttt{ggpattern} \\
    Print (B\&W)       & Greyscale-safe palette + hatching    & \texttt{sieve()} \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 295 ──
\begin{frame}{Exporting Workshop 3 Figures}
\smark{295/300}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
    \textbf{Static:}
    \begin{itemize}
        \item \texttt{ggsave("fig.pdf", width=7, height=5, dpi=300)}
        \item \texttt{fig.savefig("fig.pdf", dpi=300, bbox\_inches="tight")}
        \item PDF for journals; PNG for slides
    \end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
    \textbf{Interactive:}
    \begin{itemize}
        \item \texttt{plotly::ggplotly(p)} --- instant interactivity
        \item \texttt{htmlwidgets::saveWidget()} --- standalone HTML
        \item \texttt{pyLDAvis.save\_html()} --- topic explorer
        \item Shiny / Dash for full dashboards
    \end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{tabular}{@{}llll@{}}
    \toprule
    \textbf{Workshop} & \textbf{Title} & \textbf{Slides} & \textbf{Status} \\
    \midrule
    1 & Foundations of Data Visualization     & 300 & Complete \\
    2 & Statistical Visualization             & 300 & Complete \\
    3 & Categorical Data Visualization        & 300 & Complete \\
    4 & Time Series and Temporal Data         & 300 & Pending \\
    5 & Multivariate and High-Dimensional     & 300 & Pending \\
    6 & Geospatial Visualization              & 300 & Pending \\
    7 & Network and Graph Visualization       & 300 & Pending \\
    8 & Interactive and Animated Viz           & 300 & Pending \\
    9 & Dashboard Design and Deployment       & 300 & Pending \\
    \midrule
    \textbf{Total} & & \textbf{900 / 2{,}700} & \textbf{33\% complete} \\
    \bottomrule
\end{tabular}
\end{frame}

% ── 297 ──
\begin{frame}{Workshop 3 --- Final Takeaways}
\smark{297/300}
\begin{enumerate}
    \item \textbf{Sort nominal by value.} Preserve ordinal order.
    \item \textbf{Mosaic + residual shading} is the ``one display'' for contingency tables.
    \item \textbf{Diverging stacked bars} are the standard for Likert and ordinal ratings.
    \item \textbf{Alluvial diagrams} show how entities partition across categorical variables.
    \item \textbf{CA/MCA biplots} discover latent structure in large tables.
    \item \textbf{Effect size always}: Cram\'er's $V$, OR, CLES --- not just $p$.
    \item \textbf{Text is categorical}: frequency bars, TF-IDF, and LDA topics.
\end{enumerate}
...
\begin{column}{0.50\textwidth}
    ...
    \begin{itemize}
        \item Michael Friendly --- \texttt{vcd}, mosaic plots, association displays
        \item Michael Greenacre --- correspondence analysis
        \item Julia Silge \& David Robinson --- \texttt{tidytext}
        \item Alexander Lex --- UpSet plots
    \end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
    \textbf{Software:}
    \begin{itemize}
        \item R Core Team
        \item Hadley Wickham --- ggplot2, tidyverse
        \item Python Software Foundation
        \item The open-source visualization community
    \end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{0.50\textwidth}
    ...
    \begin{itemize}
        \item \texttt{scripts/m3\_exercises.R}
        \item \texttt{scripts/m3\_exercises.py}
        \item Each exercise has a numbered section
        \item Solutions use only Workshop 3 packages
    \end{itemize}
\end{column}
\begin{column}{0.47\textwidth}
    \textbf{Key datasets:}
    \begin{itemize}
        \item \texttt{HairEyeColor} (R built-in)
        \item Titanic (via \texttt{Stat2Data} / \texttt{seaborn})
        \item Simulated Likert data
        \item Text corpora (gutenberg / newsgroups)
    \end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\centering\vspace{1.5cm}
{\Huge\textbf{Workshop 3 Complete}}\\[0.8cm]
{\Large Categorical Data Visualization: Proportions, Association, and Composition}\\[0.5cm]
300 slides $\cdot$ 57 real plots $\cdot$ 10 modules\\[0.3cm]
Comparative R \& Python throughout\\[0.8cm]
\textbf{Next Workshop:} Workshop 4 --- Time Series and Temporal Data
\end{frame}
